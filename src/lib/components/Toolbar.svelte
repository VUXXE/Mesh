<script lang="ts">
	...
	const showContextualBar = $derived(
		activeTool === 'pen' ||
			activeTool === 'line' ||
			activeTool === 'arrow' ||
			activeTool === 'rectangle' ||
...
<div
	class="pointer-events-none fixed bottom-6 left-1/2 z-20 flex max-w-[95vw] -translate-x-1/2 flex-col items-center gap-2 select-none"
>
	<!-- Contextual Floating Bar (appears when a drawing tool is active or shapes are selected) -->
	{#if showContextualBar}
		<div
			class="no-scrollbar pointer-events-auto flex max-w-full items-center gap-1.5 overflow-x-auto rounded-xl border border-(--surface-2) bg-(--surface-1) px-2.5 py-1.5 shadow-2xl backdrop-blur-md transition-all sm:overflow-visible"
		>
			<!-- Colors Palette -->
			...
			<!-- Selection Actions (Duplicate, Bring to Front, Send to Back, Delete) -->
			{#if selectedCount > 0}
				<div class="mx-1 h-5 w-px bg-(--surface-2)"></div>
				<div class="flex items-center gap-1">
					<!-- Duplicate -->
					<button
						...
						class="rounded-lg p-1.5 text-(--ink-2) transition-colors hover:bg-(--surface-2) hover:text-(--ink-1) focus:outline-none"
					...
					<button
						...
						class="rounded-lg p-1.5 text-rose-400 transition-colors hover:bg-(--surface-2) hover:text-rose-300 focus:outline-none"
	...
	<!-- Primary Bottom Bar (Always Visible Dock) -->
	<div
		class="pointer-events-auto flex max-w-full items-center gap-1.5 overflow-visible rounded-xl border border-(--surface-2) bg-(--surface-1) px-2.5 py-2 shadow-2xl backdrop-blur-md"
	>
